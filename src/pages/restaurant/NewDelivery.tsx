...
const PRODUCT_TYPES = [
  ...
  { id: 'fragil', label: 'Produto Frágil', icon: '🍷' },
    ...
    if (settings) {
      ...
      if (batchInfo.time_remaining_minutes) {
        toast({
          ...
          description: `Você tem ${Math.ceil(
            batchInfo.time_remaining_minutes
          )} minutos para adicionar entregas a esta rota.`,
        });
      }
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (restaurant.wallet_balance < estimatedPrice) {
      toast({
        ...
        description: `Você precisa de R$ ${estimatedPrice.toFixed(
          2
        )} mas tem apenas R$ ${restaurant.wallet_balance.toFixed(2)}.`,
      });
    ...
    <SidebarProvider>
      <div className="flex w-full bg-background layout-mobile-safe no-overflow-x">
        ...
        <div className="flex-1 flex flex-col min-w-0 max-w-full">
          {/* Header */}
          <header className="sticky top-0 z-10 h-12 sm:h-14 border-b bg-background flex items-center gap-2 px-3 sm:px-4">
            <Button
              variant="ghost"
              size="icon"
            ...
            {step === 3 && !isAdditionalDelivery && (
              <div className="p-4 space-y-4">
                ...
                <VehicleCategorySelector
                  onSelect={(_id, category) => setSelectedCategory(category)}
                  ...
                  <CardContent className="p-4 space-y-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-medium leading-snug">{formatAddress(pickupAddress)}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-medium leading-snug">{formatAddress(deliveryAddress)}</p>
                        {recipientName && (
                          <p className="text-xs text-muted-foreground mt-0.5">Para: {recipientName}</p>
                        )}
                      </div>
                    </div>
          ...
          {/* Footer Button */}
          <div className="footer-button-safe flex-shrink-0 p-3 sm:p-4 border-t bg-background">
            {step < totalSteps ? (
              <Button
                size="lg"
              ...
              <Button
                ...
                disabled={
                  submitting || (restaurant ? restaurant.wallet_balance < estimatedPrice : true)
                }
              >
                {submitting ? (
                  <>
