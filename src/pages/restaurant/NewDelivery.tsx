...
import { ArrowLeft, MapPin, Navigation, Package, Check, ChevronRight, Loader2, AlertCircle, L
import LocationPicker from '@/components/LocationPicker';
import VehicleCategorySelector, { DeliveryCategory } from '@/components/VehicleCategorySelect
...
{ id: 'fragil', label: 'Produto Frágil', icon: ' { id: 'eletronico', label: 'Eletrônico', icon: ' { id: 'outro', label: 'Outro', icon: ' ' },
' },
' },
];
...
}, [distance, selectedCategory, productType, productSettings, isAdditionalDelivery, parentD
const fetchRestaurant = async () => {
if (!user) return;
const { data, error } = await supabase
.from('restaurants')
.select('id, business_name, address, latitude, longitude, wallet_balance, blocked_balan
.eq('user_id', user.id)
.single();
if (error || !data) {
...
description: 'A janela de tempo para adicionar entregas expirou ou a entrega já saiu
});
navigate('/restaurant/dashboard');
return;
}
...
description: batchInfo.reason || 'A janela de tempo expirou ou o limite de entregas f
});
navigate('/restaurant/dashboard');
return;
}
...
toast({
title: ' Entrega adicional',
description: `Você tem ${Math.ceil(batchInfo.time_remaining_minutes)} minutos para
});
}
}
};
...
description: `Você precisa de R$ ${estimatedPrice.toFixed(2)} mas tem apenas R$ ${res
});
...
? `R$ ${estimatedPrice.toFixed(2)} bloqueado. Entrega adicionada ao entregador ativ
...
{/* Header */}
<header className="sticky top-0 z-10 h-12 sm:h-14 border-b bg-background flex items
<Button
variant="ghost"
size="icon"
...
<h1 className="font-semibold flex items-center gap-2 text-sm sm:text-base trunc
{isAdditionalDelivery && (
<Badge variant="secondary" className="bg-success/10 text-success text-xs fl
...
<p className="text-[10px] sm:text-xs text-muted-foreground">Etapa {step} </div>
{/* Progress */}
...
<div className="w-16 h-16 rounded-full bg-green-100 mx-auto mb-3 flex items
<MapPin className="h-8 w-8 text-green-600" />
</div>
<h2 className="text-xl font-bold">Local de Coleta</h2>
<p className="text-sm text-muted-foreground">De onde será coletado o </div>
pacote
<Card>
...
<div className="p-4 space-y-4">
<div className="text-center mb-6">
<div className="w-16 h-16 rounded-full bg-red-100 mx-auto mb-3 flex items-c
...
<div className="text-center mb-6">
items-
<div className="w-16 h-16 rounded-full bg-blue-100 mx-auto mb-3 flex <span className="text-3xl"> </span>
</div>
<h2 className="text-xl font-bold">Tipo de Veículo</h2>
<p className="text-sm text-muted-foreground">Selecione o veículo adequado</
</div>
...
R$ {(selectedCategory.base_price + (distance * selectedCategory.price
</p>
<p className="text-xs text-muted-foreground mt-1">
Base R$ {selectedCategory.base_price.toFixed(2)} + R$ {selectedCatego
</p>
</CardContent>
</Card>
)}
...
<div className="p-4 space-y-4">
<div className="text-center mb-6">
<div className="w-16 h-16 rounded-full bg-purple-100 mx-auto mb-3 flex item
<Package className="h-8 w-8 text-purple-600" />
</div>
<h2 className="text-xl font-bold">O que será enviado?</h2>
<p className="text-sm text-muted-foreground">Isso ajuda no cuidado com sua
</div>
...
<p className="text-sm text-muted-foreground">Revise os dados antes de confi
</div>
<Card>
<CardContent className="p-4 space-y-4">
<div className="flex items-start gap-3">
<div className="w-8 h-8 rounded-full bg-green-100 flex items-center jus
<MapPin className="h-4 w-4 text-green-600" />
</div>
<div className="flex-1 min-w-0">
<p className="text-xs text-muted-foreground">Coleta</p>
<p className="text-sm font-medium leading-snug">{formatAddress(pickup
</div>
</div>
<div className="border-l-2 border-dashed border-muted ml-4 h-4" />
<div className="flex items-start gap-3">
<div className="w-8 h-8 rounded-full bg-red-100 flex items-center justi
<Navigation className="h-4 w-4 text-red-600" />
</div>
<div className="flex-1 min-w-0">
<p className="text-xs text-muted-foreground">Entrega</p>
<p className="text-sm font-medium leading-snug">{formatAddress(delive
{recipientName && (
<p className="text-xs text-muted-foreground mt-0.5">Para: {recipien
)}
</div>
</div>
...
{isAdditionalDelivery ? parentDelivery?.vehicle_type : selectedCa
</p>
</div>
<div>
<p className="text-muted-foreground">Tipo</p>
<div className="flex items-center gap-1">
<p className="font-medium">{PRODUCT_TYPES.find(p => p.id === prod
{isAdditionalDelivery && (
...
<p className="font-medium">R$ {restaurant?.wallet_balance.toFixed(2
...
Adicione R$ {(estimatedPrice - restaurant.wallet_balance).toFixed(2
</p>
</div>
</CardContent>
</Card>
...
disabled={submitting || (restaurant ? restaurant.wallet_balance < estimatedPr
>
{submitting ? (
<>
