...
iconRetinaUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/images/marker-icon-2x.
...
const [searchTimeout, setSearchTimeoutState] = useState<ReturnType<typeof setTimeout> | nul
...
`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&addr
);
const data = await response.json();
if (data.display_name) {
setAddress(data.display_name);
onLocationSelect(lat, lng, data.display_name);
// Auto-fill manual fields from reverse geocode
if (data.address) {
setStreet(data.address.road || data.address.street || '');
setNumber(data.address.house_number || '');
setNeighborhood(data.address.suburb || data.address.neighbourhood || data.address
...
attribution:
'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contribut
}).addTo(map);
...
const response = await fetch(
`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(fullAd
);
const data = await response.json();
...
// If geocoding fails, use default coordinates but save the address
setAddress(fullAddress);
onLocationSelect(latitude, longitude, fullAddress);
alert('Endereço salvo, mas não foi possível localizar no mapa. A localização aproxima
}
} catch (error) {
console.error('Erro ao buscar coordenadas:', error);
// Save address anyway with current coordinates
setAddress(fullAddress);
onLocationSelect(latitude, longitude, fullAddress);
} finally {
setSearching(false);
...
<div className="w-full aspect-[4/3] sm:h-[350px] rounded-lg border shadow-lg <div ref={mapContainerRef} className="w-full h-full" />
</div>
...
/>
{searching && (
<Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 animate-s
)}
</div>
...
key={index}
type="button"
className="w-full text-left p-2 rounded-md hover:bg-muted transition-co
